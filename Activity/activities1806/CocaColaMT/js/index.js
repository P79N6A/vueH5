...
$(document).ready(function() {
    ...
    $('.pointer').click(function() {
        $('.limit_times').html(0)
        if (typeof(localStorage.mt_imgpath)=="undefined"){
        ...
        $("#loadingToast").show();
        var getData = {}
        getData.openid = localStorage.mt_openid;
        getData.activity_id = localStorage.mt_activity_id;
        getData.store_code = localStorage.mt_store_code;
        getData.third_party_openid = sessionStorage.third_party_openid
        getData.third_party_name = localStorage.mt_third_party_name
        getData.buyer_addr = localStorage.mt_imgpath
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.return_code != 200) {
                    ...
                    localStorage.removeItem('mt_imgpath')
                    turnplate.bRotate = !turnplate.bRotate;
                    //获取奖品
                    GetSelect = data.data.coupon.id; //从后台拿数据
                    coupon_code = data.data.coupon_code;
                    localStorage['mt_coupon_code'] = coupon_code;
                    if (GetSelect == 1975) {
                        //号角瓶
...
$(".alert_gift_box").click(function(event) {
    ...
    if(item == 4){
        ...
        //关注礼遇小可
        if (localStorage.mt_subscribe == 1) {
            window.location.href = coupon_bag_host + 'webcoupon.html' + "?coupon_code=" + localStorage.mt_coupon_code + "&openid=" + localStorage.mt_openid;
        } else {
