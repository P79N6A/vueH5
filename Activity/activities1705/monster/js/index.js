...
// 存次数
var d = new Date();
var str = d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + d.getDate() + openid;
if (localStorage[str]) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.return_code != 200) {
                ...
                if (img_name.indexOf("Monster") > 0 || img_name.indexOf("买一赠一") > 0) {
                    $(".alert_gift_box").css("background-image", "url(img/Popup.png)");
                    $(".coupon_img").attr("src", "img/monster.png");
                    $(".get_coupon").css("background-image", "url(img/use.png)");
                } else {
                    $.ajax({
                        type: "POST",
                    ...
                    $(".alert_gift_box").css("background-image", "url(img/Popup_sorry.png)");
                    $(".coupon_img").attr("src", "img/ofo.png");
                    $(".get_coupon").css("background-image", "url(img/look.png)");
                }
                $(".show_box").show();
