...
$(document).ready(function() {
    //动态添加大转盘的奖品与奖品区域背景颜色
    turnplate.restaraunts = [ "川味麻小10只", "谢谢参与", "世界杯主题||玻璃杯1个", "雪碧或||可口可乐1听", "世界杯球衣1件", "世界杯纪念版||足球1个"];
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.return_code != 200) {
                    ...
                    if (GetSelect.indexOf("球衣") > 0) {
                        $(".alert_gift_box").css({ "background-image": "url(images/tedeng.png)", "top": "5%" });
                        item = 5;
                    }else if (GetSelect.indexOf("足球") > 0) {
                        $(".alert_gift_box").css({ "background-image": "url(images/yideng.png)", "top": "5%" });
                        item = 6;
                    } else if (GetSelect.indexOf("麻小") > 0) {
                        $(".alert_gift_box").css({ "background-image": "url(images/erdeng.png)", "top": "5%" });
                        item = 1;
                    } else if (GetSelect.indexOf("玻璃杯") > 0) {
                        $(".alert_gift_box").css({ "background-image": "url(images/sandeng.png)", "top": "5%" });
                        item = 3;
                    } else if (GetSelect.indexOf("听") > 0) {
                        $(".alert_gift_box").css({ "background-image": "url(images/sideng.png)", "top": "5%" });
                        item = 4;
                    } else {
                        $(".alert_gift_box").css({ "background-image": "url(images/xiexie.png)", "top": "5%" });
                        item = 2;
...
$(".alert_gift_box").click(function(event) {
    if(item == 2){
        ...
        window.location.href = coupon_bag_host + 'webcouponshow_normal_qr.html' + "?coupon_code=" + coupon_code + "&openid=" + urlGet()['openid'];
    }
});
