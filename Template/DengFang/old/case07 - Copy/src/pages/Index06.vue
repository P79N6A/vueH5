    ...
    methods: {
      ...
      createdImg(){
        ...
        function createPano(imgs, rect) {
          ...
          const step = 7 * self.fontSize;
          const radius = Math.floor(step / 2 / Math.tan(Math.PI / self.bubbleArr.length)) - 1;
          console.log(step, radius)
          for (let k = 0; k < self.bubbleArr.length; k++) {
            const item = self.bubbleArr[k];


            const _p = new C3D.Plane();
            let _r = 360 / self.bubbleArr.length;
            console.log(_r)
            const _a = Math.PI * 2 / self.bubbleArr.length * item.x;
            _p.size(140, 100).position(Math.sin(_a) * radius, item.y, -Math.cos(_a) * radius)
              .rotation(0, -_r, 0)
              .material({
        ...
        this.pano.position(0, 0, -600).updateT();
        ...
        function go() {
          i++;
          if (i <= 180) {
            self.pano.rotate(0, 5, 0).updateT();
