...
const getcouponlist = (openid, activity_id) => {
  let return_type = {};
  if (openid && activity_id && false) {
    const getCouponList = {
      "akk_openid": openid,
    ...
    $.ajax({
      type: "POST",
      url: "http://ab.aikaka.com.cn/Acmp/AcmpApi/couponRecord/list",
      data: getCouponList,
      dataType: "json",
      async: false,
      success: function (data) {
        if (data.return_code != 200) {
          ...
          for (var i = 0; i < data.data.list.length; i++) {
            ...
            var require_coupon = {};
            require_coupon.logo_img_src = "http://ab.aikaka.com.cn/Acmp/AcmpApi/" + this_coupon_info.local_logo_path;
            ...
            require_coupon.go_url = "http://ab.aikaka.com.cn/Coupon/show_webcoupon?coupon_code=" + this_coupon_info.coupon_code + "&openid=" + openid;
  ...
  /* return return_type = {
   ...
   "return_msg": "",
   "return_couponlist": [{
   "logo_img_src": "./static/data/images/intact.png", //logo图片的地址
   "title": "可口可乐", //标题
   "sub_title": "买一赠一", //副标题
