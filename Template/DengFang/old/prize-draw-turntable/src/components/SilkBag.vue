          ...
          <scroller class="record-list">
            <div class="record-one" v-for="item of couponList" v-if="couponList.length > 0 ">
              ...
              <div class="record-one-parent" @click="toLink(item.go_url)">
                ...
                <div class="record-one-status" v-if="parseInt(item.status) !== 2"
                     :class="parseInt(item.status) === 1?'record-one-status-overdue':'record-one-status-consumed'"></div>
              </div>
            </div>
            <div v-if="couponList.length <= 0 " class="record-none">暂无中奖纪录</div>
          </scroller>
...
<script>
  ...
  export default {
    ...
    created(){
      ...
      const couponlist = getcouponlist(openId, activityId);
      if (couponlist.return_code !== '200') {
        ...
        this.couponList = applied.concat(consumed, overdue);
      }
    },
    methods: {
      hideSilkBag(){
