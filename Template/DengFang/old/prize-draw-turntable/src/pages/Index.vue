  ...
  export default {
    ...
    data() {
      return {
        ...
        prizeInfo: {},
        // ---------------------私有属性-----------------------
        showArr: [30, 90, 150, 210, 270, 330], //奖盘排列序号,
    ...
    methods: {
      ...
      createdCtx(){        //画奖盘
        ...
        if (prizeDisc) {
          ...
          for (let i = 0; i < 6; i++) {
            ...
            ctx.drawImage(img, -2 * self.fontSize, 4 * self.fontSize, 4 * self.fontSize, 4 * self.fontSize);
            ctx.closePath();
            ctx.restore();
          }
        }
      ...
      draw(){        // 获取奖项结果
        ...
        if (!this.hasGetPrize) {
          ...
          const prizeBack = getcoupon(this.openId, this.activityId, this.xjjOpenId, 'third_name');
          if (prizeBack.return_code !== '200') {
            alert('获取数据失败！');
            this.tryAgain();
          } else {
            this.prizeInfo = prizeBack;
            let prizeArr = [];
            const prizeId = parseInt(prizeBack.item_id);
            for (let index in this.imagesArr) {
              if (this.imagesArr[index].id === prizeId) {
                prizeArr.push(index);
              }
            }
            this.prize = Math.random() > 0.5 ? parseInt(prizeArr[1]) : parseInt(prizeArr[0]);
      ...
      drawAction(){
        const self = this;
        const rand_circle = Math.ceil(Math.random() * 3) + 3;
